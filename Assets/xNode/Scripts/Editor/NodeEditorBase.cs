...
namespace XNodeEditor.Internal
{
    ...
    public abstract class NodeEditorBase<T, A, K> where A : Attribute, NodeEditorBase<T, A, K>.INodeEditorAttrib where T : NodeEditorBase<T, A, K> where K : ScriptableObject
    {
        ...
        //获取自定义的Editor
        public static Type GetCustomEditor(K target, bool inherit = false)
        {
            Type type = target.GetType();
        ...
        public static void RemoveEditor(K target)
        {
            ...
            NodeEditorGUILayout.RemoveTargetreorderableList(target);
        }
        ...
        public static void ClearAllCache()
        {
            if(editors.Count > 0)
                editors.Clear();
            NodeEditorGUILayout.ClearcachereorderableList();
        }
