            ...
            list.drawElementCallback =
                (Rect rect, int index, bool isActive, bool isFocused) => {
                    ...
                    if (hasArrayData && arrayData.propertyType != SerializedPropertyType.String) {
                        ...
                        //自定义显示动态端口名字
                        if (fi.IsDefined(typeof(XNode.Node.NodeDynamicPortKeyAttribute)))
                        {
                            string key = fi.GetCustomAttribute<XNode.Node.NodeDynamicPortKeyAttribute>().key;
                            EditorGUI.PropertyField(rect, itemData, new GUIContent(key), true);
                        }
                        else
